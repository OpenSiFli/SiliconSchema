...
def generate_pinctrl_header(model_id: str, pinmux_data: dict, pinr_data: dict) -> str:
    ...
    lines.append("/**")
    lines.append(" * @file")
    lines.append(f" * @brief Devicetree pin control helpers for SiFli {model_id}")
    lines.append(f" * @ingroup pinctrl_{model_id.lower()}")
    lines.append(" */")
    lines.append("")
    lines.append("/**")
    lines.append(" * @addtogroup sifli_pinctrl SiFli pin control helpers")
    lines.append(" * @ingroup devicetree-pinctrl")
    lines.append(" */")
    lines.append("")
    lines.append("/**")
    lines.append(f" * @defgroup pinctrl_{model_id.lower()} SiFli {model_id} pin control helpers")
    lines.append(f" * @brief Macros for pin control configuration of SiFli {model_id}")
    lines.append(" * @ingroup sifli_pinctrl")
    lines.append(" *")
    lines.append(" * The macros follow the following naming convention:")
    lines.append(" * @c \\<PAD\\>_\\<SIGNAL\\>.")
    lines.append(" *")
    lines.append(" * For example, @c PA19_USART1_TXD corresponds to the @c USART1_TXD signal")
    lines.append(" * routed to pad @c PA19.")
    lines.append(" *")
    lines.append(" * @code{.dts}")
    lines.append(f" * #include <zephyr/dt-bindings/pinctrl/sifli/{model_id.lower()}-pinctrl.h>")
    lines.append(" *")
    lines.append(" * &pinctrl {")
    lines.append(" *         usart1_default: usart1_default {")
    lines.append(" *                 group1 {")
    lines.append(" *                         pinmux = <PA19_USART1_TXD>;")
    lines.append(" *                 };")
    lines.append(" *                 group2 {")
    lines.append(" *                         pinmux = <PA18_USART1_RXD>;")
    lines.append(" *                         input-enable;")
    lines.append(" *                         bias-pull-up;")
    lines.append(" *                 };")
    lines.append(" *         };")
    lines.append(" * };")
    ...
    lines.append(" */")
    lines.append("")
    lines.append("/** @cond INTERNAL_HIDDEN */")
    lines.append("")
